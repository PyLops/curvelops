                  ...
                  int ac)
{
    ...
    fdct3d_param(
      m, n, p, nbscales, nbangles_coarse, ac, fxs, fys, fzs, nxs, nys, nzs);
    return py::make_tuple(fxs, fys, fzs, nxs, nys, nzs);
}

vector<vector<py::array_t<cpx>>>
fdct3d_forward_wrap(int nbscales,
                    int nbangles_coarse,
                    int ac,
                    py::array_t<cpx> x)
{
    // Our wrapper takes a NumPy array, but ``fdct3d_forward`` requires a
    // CpxNumTns input (which will be accessed read-only). So we must create
    // CpxNumTns ``xtns`` which will "mirror" our input ``x`` in a no-copy
    // fashion. We also need to output ``c`` whose conversion to a Python list
    // of lists of CpxNumTns will be handled by pybind11. The vector -> list
    // casting is automatic in pybind11, whereas the CpxNumTns ->
    ...
    fdct3d_forward(
      xtns._m, xtns._n, xtns._p, nbscales, nbangles_coarse, ac, xtns, ctns);
    ...
    for (size_t i = 0; i < ctns.size(); i++) {
        ...
        for (size_t j = 0; j < ctns[i].size(); j++) {
            // Create capsule linked to `ctns[i][j]._data` to track its
            // lifetime
            // https://stackoverflow.com/questions/44659924/returning-numpy-arrays-via-pybind11
            py::capsule free_when_done(ctns[i][j].data(), [](void* cpx_ptr) {
                    ...
                    vector<vector<py::array_t<cpx>>> c)
{
    ...
    // ``ctns``) are not compatible with the other parameters, this function
    // WILL segfault
...
PYBIND11_MODULE(fdct3d_wrapper, m)
{
    ...
    m.def("fdct3d_param_wrap",
          &fdct3d_param_wrap,
          "Parameters for 3D FDCT",
          py::return_value_policy::take_ownership);
    m.def("fdct3d_forward_wrap",
          &fdct3d_forward_wrap,
          "3D Forward FDCT",
          py::return_value_policy::take_ownership);
    m.def("fdct3d_inverse_wrap",
          &fdct3d_inverse_wrap,
          "3D Inverse FDCT",
          py::return_value_policy::take_ownership);
}
